...
function typeLabelForWarning(t) {
...
function parseSkillYaml(yamlText) {
  const lines = yamlText.split(/\r?\n/)
  const markers = {
    disable: { exists: false, isBoolean: false, value: undefined, wrongType: false, wrongTypeDetail: undefined, ambiguous: false },
    metadata: { exists: false, isBoolean: false, value: undefined, wrongType: false, wrongTypeDetail: undefined, ambiguous: false },
  }
  const warnings = []
  let malformed = false
  let malformedMsg = null
  const seen = new Map()
  const stack = []
  ...
  for (let idx = 0; idx < lines.length; idx++) {
    ...
    if (fullPath === "metadata" && valueWithoutComment.startsWith("{")) {
      ...
      if (inner) {
        let innerRaw = stripComment(inner[1]).trim()
...
function parseSidecarYaml(yamlText) {
  const lines = yamlText.split(/\r?\n/)
  const markers = {
    sidecar: { exists: false, isBoolean: false, value: undefined, wrongType: false, wrongTypeDetail: undefined, ambiguous: false },
  }
  const warnings = []
  let malformed = false
  let malformedMsg = null
...
export function classifyRecord({ skillContent, skillReadError, sidecarContent, sidecarReadError, sidecarMissing }) {
  const warnings = []
  ...
  if (skillReadError) {
    const msg = skillReadError && skillReadError.message ? skillReadError.message : String(skillReadError)
  ...
  if (sidecarReadError) {
    const msg = sidecarReadError && sidecarReadError.message ? sidecarReadError.message : String(sidecarReadError)
  ...
  return { explicitOnly, warnings: deduped, details: { disableDenies, metadataDenies, sidecarDenies, disableAllows, metadataAllows, sidecarAllows } }
}

// Backwards alias for tests that import classify
export const classify = classifyRecord
export { parseSkillYaml, parseSidecarYaml }
